...
<script>
        jQuery(document).ready(function ($) {
            ...
            <?php 
                if ( is_array( $this->get("Transition") ) ) {
                    $transitions = implode(",", $this->get("Transition"));
                }
            ?>

            <?php if ( isset( $transitions ) && $transitions != "" )  { ?>
            var _SlideshowTransitions = [ <?php echo $transitions ?> ];
            <?php } ?>
            ...
            var options = { 
                ...
                <?php echo $this->get_js_option("DragOrientation", ",\n") ?>
                <?php echo $this->get_js_option("ArrowKeyNavigation", ",\n") ?>
                ...
                <?php if ( $this->get("ShowThumbnails") > 0 )  { ?>
                 ,$ThumbnailNavigatorOptions: {                       //[Optional] Options to specify and enable thumbnail navigator or not
                    $Class: $JssorThumbnailNavigator$,              //[Required] Class to create thumbnail navigator instance
                    $ChanceToShow: <?php echo $this->get("ShowThumbnails") ?>,//[Required] 0 Never, 1 Mouse Over, 2 Always
...
<div id="zzis_container_<?php echo $zzis_post->ID ?>" class="zzis_container" style="position: relative; overflow: hidden; top: 0px; left: 0px; width: <?php echo $this->get("SlideWidth") ?>px; height: <?php echo $this->get("SlideHeight") ?>px;">    
    <div u="slides" style="cursor: move; position: absolute; overflow: hidden; left: 0px; top: 0px; width: <?php echo $this->get("SlideWidth") ?>px; height: <?php echo $this->get("SlideHeight") ?>px;">
        ...
        <?php foreach ($images as $img) { ?>
            ...
            <div class="zz-slide-<?php echo $i ?>">
                ...
                <?php if ( $this->get("ShowThumbnails") > 0 )  { ?>
                <img u="thumb" class="zzis-slide-thumb" src="<?php echo $img['zzis_image_url'] ?>" />
                <?php } ?>
    ...
    <?php 
        ...
        if ( $this->get("ShowThumbnails") > 0 )  { 
            $this->get_thumbnails_skin_hmtl ( $this->get( "ThumbnailsSkin" ) );
        }
    ?>
