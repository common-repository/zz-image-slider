    ...
    <div class="zzis-admin-inner-div2" >
        ...
        <p>
            ...
            <select name="zzis_image_readmore_link_type[]" id="zzis_image_readmore_link_type[]">
                <option value="0" <?php echo ((!isset($readmorelink_type) || $readmorelink_type == "0") ? "selected" : "") ?> ><?php _e("button", ZZIS_TEXT_DOMAIN) ?></option>
                <option value="1" <?php echo ($readmorelink_type == "1" ? "selected" : "") ?>><?php _e("image", ZZIS_TEXT_DOMAIN) ?></option>
            </select>
        </p>
        <p>
            <label><?php _e("Visible", ZZIS_TEXT_DOMAIN) ?></label>
            <input type="checkbox" <?php echo ( $visible=="1" ? "CHECKED" : "") ?> onchange="jQuery( this ).next().val( this.checked ? '1' : '0' );" />
            <input name="zzis_image_visible[]" type="hidden" value="<?php echo $visible ?>" />
        </p>
    </div>
</li>
